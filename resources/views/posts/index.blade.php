...
<body class='bg-gray-100'>
    <header class='grid grid-cols-12 gap-x-6 px-2 py-3 bg-white rounded-b-lg'>
     ...
     <section class="grid grid-cols-12 gap-x-6 my-2 auto-rows-min">
                        ...
                        @foreach($posts as $post)
                            <article class="space-y-4 p-4 bg-white rounded-2xl">
                                ...
                                <div class='flex items-center space-x-2 '>
                                    <div class='flex pl-5'>
                                        <div class = ' -ml-4 p-1' >
                                            <div  > <img src="{{ $post->user->profile_photo_path ? asset('storage/' . $post->user->profile_photo_path) : asset('storage') . '/default/default.png' }}"  style="clip-path:circle()"  class="w-7"></div>
                                        </div>
    
                                         <div class = ' -ml-4  p-1'>
                                            <div style="clip-path:circle()"> <img src="{{ $post->user->profile_photo_path ? asset('storage/' . $post->user->profile_photo_path) : asset('storage') . '/default/default.png' }}"  class="w-7"></div>
                                        </div>
                                    ...
                                    <p class='text-lg'>liked by <strong>emlia achiever</strong> and <strong>2,357 others</strong> </p>
                                </div>
                                
                                <!-- caption  -->
                                <div>
                                    <p class="text-gray-600"> <strong class="text-black pr-2">{{$post->user->name}}</strong>{{$post->caption}}</p>
                                </div>
                            </article>
                        @endforeach
                        {{ $posts->links() }}
            ...
            <!-- side right  -->
            <aside class="bg-slate-300 col-span-3 row-span-2 rounded-2xl">
            </aside>
        </section>
</body>
</html>